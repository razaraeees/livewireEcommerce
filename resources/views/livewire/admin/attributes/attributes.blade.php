    ...
    <div class="dashboard-page-content">
        <div class="row mb-9 align-items-center justify-content-between">
            <div class="col-md-6 mb-8 mb-md-0">
                <h2 class="fs-4 mb-0">Attributes List</h2>
                <p>Manage all your product Attributes</p>
            </div>
            ...
            <div class="card-header bg-transparent px-0 pt-0 pb-7">
                <div class="row align-items-center justify-content-between">
                    <div class="col-md-3 col-12 mr-auto mb-md-0 mb-6">
                        <input type="text" wire:model.live.debounce.300ms="search" placeholder="Search attribute..."
                    ...
                    <table class="table table-hover align-middle table-nowrap mb-0">
                        ...
                        <tbody>
                            @forelse ($attributes as $item)
                                <tr>
                                    <td>{{ $loop->iteration + ($attributes->currentPage() - 1) * $attributes->perPage() }}
                                    </td>
                                    <td>
                                        <span class="text-dark">{{ $item->name }}</span>
                                    </td>
                                    <td>
                                        @if($item->attributeValue && $item->attributeValue->count() > 0)
                                            @foreach($item->attributeValue as $val)
                                                @php
                                    ...
                                    <td class="text-center">
                                        <div class="d-flex flex-nowrap justify-content-center">
                                            <a href="{{ route('admin.attribute.edit', $item->slug) }}"
                            ...
                            @empty
                               <tr>
                                    <td colspan="6" class="text-center">No Attribute found.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
        ...
        @if ($attributes->hasPages())
            <nav aria-label="Page navigation example" class="mt-6 mb-4">
                <ul class="pagination justify-content-start">
                    ...
                    @foreach ($attributes->links()->elements[0] as $page => $url)
