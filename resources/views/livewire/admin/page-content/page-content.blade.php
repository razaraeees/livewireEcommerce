    ...
    <div class="dashboard-page-content">
        <div class="row mb-9 align-items-center justify-content-between">
            <div class="col-md-6 mb-8 mb-md-0">
                <h2 class="fs-4 mb-0">Page Content</h2> {{-- Updated title --}}
                <p>Manage your page content here.</p>
            </div>
                ...
                <div class="table-responsive">
                    <table class="table table-hover align-middle table-nowrap mb-0">
                        <thead>
                            <tr>
                                <th>S.No</th>
                                <th>Type</th> {{-- Added Type field --}}
                                <th>Title</th> {{-- Changed from Brand to Title --}}
                                <th>Slug</th>
                                <th>Status</th>
                                <th>Created At</th>
                        ...
                        <tbody>
                            @forelse ($items as $item)
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <span class="badge bg-info">{{ $item->type }}</span> {{-- Display type --}}
                                    </td>
                                    <td>
                                        <div class="d-flex align-items-center flex-nowrap">
                                            {{-- If you have images, uncomment and modify this section --}}
                                            {{-- <a href="#" title="{{ $item->title }}">
                                                    <img src="{{ asset('storage/' . $item->image) }}" data-src="{{ asset('storage/' . $item->image) }}"
                                                        alt="{{ $item->title }}" class="lazy-image" width="100" height="70">
                                                </a> --}}
                                            <a href="#" name="{{ $item->title }}" class="ms-6">
                                                <p class="fw-semibold text-body-emphasis mb-0">{{ $item->title }}</p>
                                                {{-- Optional: Show truncated content --}}
                                                @if ($item->content)
                                                    <small
